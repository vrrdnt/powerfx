...
export interface Token { kind: 'word' | 'number' | 'string' | 'identifier' | 'symbol' | 'comment'; text: string; from: number; to: number }
...
export function lex(source: string, locale: Locale = 'dot', cloud = false): Token[] {
  const result: Token[] = [];
  let i = 0;
  const fail = (message: string, at = i): never => { throw new SyntaxFailure(message, at, Math.min(source.length, at + 1)); };
  function quoted(quote: string, start: number): void {
    ...
    fail(`Unclosed ${quote === '"' ? 'text string' : cloud ? 'text string' : 'quoted identifier'}.`, start);
  }
  function interpolated(start: number): void {
    ...
    while (i < source.length) {
      ...
      if (source.slice(i, i + 2) === '{{' || source.slice(i, i + 2) === '}}') { i += 2; continue; }
  ...
  while (i < source.length) {
    ...
    if (source.slice(i, i + 2) === '//' && !cloud) {
      ...
      kind = 'comment'; const end = source.indexOf('*/', i + 2);
      if (end < 0) fail('Unclosed block comment.', from);
      i = end + 2;
    } else if (source.slice(i, i + 2) === '$"' && !cloud) {
      kind = 'string'; interpolated(from);
    } else if (c === '"' && !cloud || c === "'") {
      kind = c === "'" && !cloud ? 'identifier' : 'string'; quoted(c, from);
    } else if (/[0-9]/.test(c) || c === (locale === 'comma' && !cloud ? ',' : '.') && /[0-9]/.test(source[i + 1] ?? '')) {
      kind = 'number';
      const decimal = locale === 'comma' && !cloud ? ',' : '.';
      while (/[0-9]/.test(source[i] ?? '')) i++;
      if (source[i] === decimal) { i++; while (/[0-9]/.test(source[i] ?? '')) i++; }
      if (/[eE]/.test(source[i] ?? '')) {
        i++; if (/[+-]/.test(source[i] ?? '')) i++;
        const exponent = i; while (/[0-9]/.test(source[i] ?? '')) i++;
        if (i === exponent) fail('Expected digits after the exponent.', from);
      }
    } else if (wordStart.test(c)) {
      kind = 'word'; i++; while (i < source.length && wordRest.test(source[i])) i++;
    } else {
      const pair = source.slice(i, i + 2);
      if (['<=', '>=', '<>', '&&', '||', ':='].includes(pair) || pair === ';;' && locale === 'comma' && !cloud) i += 2;
...
export function findInterpolationEnd(source: string, start: number, cloud: boolean): number {
  ...
  for (let i = start; i < source.length; i++) {
    ...
    if (c === "'" || c === '"' && !cloud) {
      ...
      if (!closed) throw new SyntaxFailure('Unclosed string inside interpolation.', start, source.length);
    } else if (!cloud && source.slice(i, i + 2) === '//') {
      while (i < source.length && !/[\r\n]/.test(source[i])) i++;
    } else if (!cloud && source.slice(i, i + 2) === '/*') {
      const end = source.indexOf('*/', i + 2);
      if (end < 0) throw new SyntaxFailure('Unclosed comment inside interpolation.', i, source.length);
      i = end + 1;
    } else if (c === '{') depth++;
