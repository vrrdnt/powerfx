...
export function validate(tokens: Token[], source: string, locale: Locale, cloud = false): void {
  ...
  const error = (message: string): never => { throw new SyntaxFailure(message, current()?.from ?? source.length, current()?.to ?? source.length); };
  const eat = (s: string) => { if (at(s)) { pos++; return true; } return false; };
  const need = (s: string) => { if (!eat(s)) error(`Expected ${s}.`); };
  const name = () => { if (!current() || !['word', 'identifier'].includes(current().kind)) error('Expected an identifier.'); pos++; };
  const precedence: Record<string, number> = { Or: 1, '||': 1, And: 2, '&&': 2, '=': 3, '<>': 3, '<': 3, '>': 3, '<=': 3, '>=': 3, in: 3, exactin: 3, '&': 4, '+': 5, '-': 5, '*': 6, '/': 6, '^': 7, As: 0 };
  function expression(min = 0): void {
    ...
    if (!cloud && ['-', '+', '!', 'Not'].includes(token.text) || cloud && token.text === '-') { pos++; expression(7); }
    else if (eat('(')) { sequence(); need(')'); }
    else if (!cloud && eat('{')) {
      if (!eat('}')) {
        do { name(); need(':'); expression(); } while (eat(list));
        need('}');
      }
    } else if (!cloud && eat('[')) {
      if (eat('@')) { name(); need(']'); }
      else if (!eat(']')) { do { expression(); } while (eat(list)); need(']'); }
    ...
    while (current()) {
      if (eat('(')) {
        if (!eat(')')) { do { cloud ? expression() : sequence(); } while (eat(list)); need(')'); }
      } else if (eat('.')) { name(); }
      else if (cloud && eat('?')) {
        if (!at('[') && !at('.')) error('Expected a property lookup after ?.');
      } else if (eat('[')) {
        if (cloud) expression(); else { need('@'); name(); }
        need(']');
      } else if (!cloud && eat('%')) { /* postfix percentage */ }
      else {
        const op = current().text;
        const prec = cloud ? undefined : precedence[op];
        if (prec === undefined || prec < min) break;
        pos++;
        if (op === 'As') name(); else expression(prec + (op === '^' ? 0 : 1));
      }
    }
    nesting--;
  }
  ...
  function type(): void {
    if (eat('[')) { type(); need(']'); }
      ...
      if (!eat('}')) { do { name(); need(':'); type(); } while (eat(list)); need('}'); }
    } else name();
  }
  ...
  function definition(): void {
    ...
    if (eat('(')) {
      if (!eat(')')) { do { name(); need(':'); type(); } while (eat(list)); need(')'); }
      need(':'); type(); need('=');
      if (eat('{')) { if (!at('}')) sequence(); need('}'); } else expression();
    } else { if (!eat(':=')) need('='); expression(); }
    need(';');
  }
  if (!ts.length) return;
  // A final semicolon distinguishes a named-formula document from an equality expression.
  const definitions = !cloud && (isFunctionDefinition() || ts[1]?.text === ':=' || ts[1]?.text === '=' && ts.at(-1)?.text === ';');
  if (definitions) { while (current()) definition(); }
  else sequence();
...
function validatePowerTemplate(token: Token, locale: Locale): void {
  ...
  for (let i = 2; i < text.length - 1; i++) {
    if (text.slice(i, i + 2) === '{{' || text.slice(i, i + 2) === '}}') { i++; continue; }
    if (text[i] !== '{') continue;
    const end = findInterpolationEnd(text, i + 1, false);
    const expression = text.slice(i + 1, end);
    try {
      if (!expression.trim()) throw new SyntaxFailure('Expected an expression inside interpolation.', 0, expression.length);
      validate(lex(expression, locale), expression, locale);
    } catch (e) {
      if (e instanceof SyntaxFailure) throw new SyntaxFailure(e.message, token.from + i + 1 + e.from, token.from + i + 1 + e.to);
      throw e;
    }
    i = end;
  }
