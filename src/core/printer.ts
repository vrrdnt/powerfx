...
type Doc =
  | string
  | { type: 'line'; soft?: boolean; hard?: boolean }
  | { type: 'indent' | 'group'; content: Doc[]; force?: boolean };
const line: Doc = { type: 'line' },
  soft: Doc = { type: 'line', soft: true },
  hard: Doc = { type: 'line', hard: true };
  ...
  function tree(end?: string): Item[] {
    ...
    while (index < tokens.length) {
      ...
      if (closer && t.kind === 'symbol') {
        const children = tree(closer);
        items.push({ open: t, close: tokens[index - 1], children });
  ...
  function docs(items: Item[], parent = ''): Doc[] {
    ...
    for (let k = 0; k < items.length; k++) {
      ...
      if (prev && !separated) {
        ...
        /\r?\n[\t ]*\r?\n/.test(source.slice(last(prev).to, t.from))
      )
        result.push(hard);
      separated = false;
      if (item.children) {
        const open = item.open!.text,
        ...
        const isLookup =
          open === '[' && ((cloud && !!prev) || item.children[0].token?.text === '@');
        const call = open === '(' && !!prev && ['word', 'identifier'].includes(last(prev).kind);
        const record = open === '{';
        const expanded =
          options.preset === 'expanded' ||
          (options.preset === 'custom' &&
            (record ? options.recordLayout : options.argumentLayout) === 'expanded');
        const semanticCall =
          options.preset === 'adaptive' &&
          call &&
          ['If', 'Switch', 'With', 'Patch', 'ForAll', 'Concurrent'].includes(last(prev).text) &&
          item.children.some((child) => child.children);
        const forced =
          !isLookup && (semanticCall || (expanded && (call || record || open === '[')));
        ...
        result.push(t.text);
        if (
          items[k + 1]?.token?.kind === 'comment' &&
          !/[\r\n]/.test(source.slice(t.to, first(items[k + 1]).from))
        )
          result.push(' ');
...
function render(docs: Doc[], options: FormatOptions): string {
  ...
  function fits(content: Doc[], remaining: number): boolean {
    ...
    while (pending.length && remaining >= 0) {
      ...
      if (typeof d === 'string') {
        ...
        if (d.force) return false;
        for (let i = d.content.length - 1; i >= 0; i--) pending.push(d.content[i]);
      }
    }
    return remaining >= 0;
  }
  while (stack.length) {
    ...
    if (typeof doc === 'string') {
      output += doc;
      if (doc.trim()) protectedEnd = output.length;
      ...
      if (flat && !doc.hard) {
        ...
        output = output.slice(0, protectedEnd) + output.slice(protectedEnd).replace(/[\t ]+$/, '');
        output += '\n' + unit.repeat(level);
        column = level * options.indentSize;
      }
    } else {
      const nextFlat =
        doc.type === 'group' ? !doc.force && fits(doc.content, width - column) : flat;
      for (let i = doc.content.length - 1; i >= 0; i--)
        stack.push({
          doc: doc.content[i],
          level: level + (doc.type === 'indent' ? 1 : 0),
          flat: nextFlat,
        });
    }
  }
  return output.slice(0, protectedEnd);
}
