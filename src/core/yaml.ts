...
export function formatYaml(
...
): { text: string; outline: OutlineEntry[] } {
  ...
  function visit(node: Node | null, path: string[], property = false): void {
    if (isMap(node)) {
      ...
      const from = range[0],
        to = range[1];
      const label = path
        .filter((p) => !/^\d+$/.test(p) && !['Properties', 'Children', 'Screens'].includes(p))
        .join(' › ');
      ...
      try {
        formatted =
          '=' +
          formatFormula(node.value.slice(1), { ...options, mode: 'powerapps', locale: 'dot' });
      } catch (error) {
        if (error instanceof SyntaxFailure)
          throw new SyntaxFailure(`${label}: ${error.message}`, from, to);
        throw error;
      }
      ...
      if (node.type === 'QUOTE_DOUBLE') replacement = JSON.stringify(formatted);
        ...
        replacement = formatted.includes('\n')
          ...
          : "'" + formatted.replace(/'/g, "''") + "'";
      else if (!formatted.includes('\n') && !/:\s|\s#/.test(formatted) && node.type === 'PLAIN')
        ...
        const body = formatted
          ...
          .join(newline);
        replacement =
          indicator + headerComment + newline + body + newline.repeat(Math.max(1, trailing));
  ...
  if (check.errors.length)
    throw new SyntaxFailure(
      'The formatted YAML could not be safely reconstructed. The original has been kept.',
      0,
      source.length,
    );
    ...
    if (node && typeof node === 'object')
      return Object.fromEntries(
        Object.entries(node).map(([k, v]) => [
          k,
          stripFormulaWhitespace(v, inProperties || k === 'Properties'),
        ]),
      );
    if (inProperties && typeof node === 'string' && node.startsWith('='))
      return {
        formulaTokens: lex(node.slice(1), 'dot').map((t) => t.text),
        finalNewlines: node.match(/(?:\r?\n)+$/)?.[0] ?? '',
      };
    return node;
  };
  if (
    JSON.stringify(stripFormulaWhitespace(document.toJS({ maxAliasCount: 100 }))) !==
    JSON.stringify(stripFormulaWhitespace(check.toJS({ maxAliasCount: 100 })))
  ) {
    throw new SyntaxFailure(
      'YAML values changed unexpectedly. The original has been kept.',
      0,
      source.length,
    );
  }
