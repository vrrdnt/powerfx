...
function nested(
  ...
  try {
    ...
    if (error instanceof SyntaxFailure)
      throw new SyntaxFailure(error.message, start + error.from, start + error.to);
    throw error;
  }
}
export function formatTemplate(source: string, options: FormatOptions, cloud: boolean): string {
  ...
  if (!cloud && trimmed.startsWith('='))
    return '=' + nested(source, offset + 1, offset + trimmed.length, options, false);
  ...
  for (let i = 0; i < source.length; i++) {
    if (cloud && source.slice(i, i + 2) === '@@') {
      i++;
      continue;
    }
    if (!cloud && source.slice(i, i + 3) === '$${') {
      i += 2;
      continue;
    }
