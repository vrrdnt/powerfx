...
describe('Power Apps formatter', () => {
  ...
  it('bounds nesting', () =>
    expect(format('('.repeat(250) + '1' + ')'.repeat(250)).diagnostics[0]?.message).toContain(
      'deeply',
    ));
  ...
  it('does not require app bindings', () =>
    expect(
      format('Company.CustomFunction(UnknownDataSource, MissingVariable)').diagnostics,
    ).toEqual([]));
});
