...
describe('Power Automate', () => {
  ...
  for (const [mode, source] of cases) for (const preset of ['adaptive','expanded','compact'] as const) {
  ...
  it('does not parse Power Apps as WDL', () => expect(format('If(x > 1,"yes","no")', {mode:'cloud'}).diagnostics.length).toBeGreaterThan(0));
});
...
describe('Power Apps YAML', () => {
  ...
  for (const source of sources) it(`round trips ${source.slice(0,80)}`, () => {
